...
def write_lammps_input_langevin(filament_name: filament, box_dimensions: list, create_membrane: bool, boundary_conditions: list, mass: list, bond_styles:list, angle_styles:list, pair_coeff:list, pair_cutoffs:list, groups:list, sim_parameters:list, folders:list, langevin_parameters:list, input_fname_str: str, filament_datafile:str, dump_minimization: bool, fix_nve_min: list, fix_nve_run:list, fix_wall: list, shake_parameters: list):
    ...
    temperture_start, temperature_stop = temperture
    
    langevin_seed, langevin_damp = langevin_parameters
    ...
    with open(input_fname_str, 'w') as input_f:
        ...
        input_f.write("variable temperature_min equal {:.4f}\n".format(temperture_start))
        input_f.write("variable temperature_max equal {:.4f}\n".format(temperature_stop))
        
        input_f.write("\n")
        
        input_f.write("variable langevin_seed equal {:d}\n".format(langevin_seed))
        input_f.write("variable langevin_damp equal {:.4f}\n".format(langevin_damp))
        ...
        bc1, bc2, bc3 = boundary_conditions
        ...
        if fix_nve_run != []:
            input_f.write(
                "fix {} all nve/limit {:.6f}\n".format(fix_nve_run[0], fix_nve_run[1]))
        else:
            input_f.write("fix nverun all nve\n")
        ...
        input_f.write(
            "fix langevinfix all langevin ${temperature_min} ${temperature_max} ${langevin_damp} ${langevin_seed}\n")
