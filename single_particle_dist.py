...
try:
    data_file = 'link_pos/link_pos_single.{}.txt'.format(run_i)
...
distances_list = np.zeros((num_iterations, 1))
...
if plot_traces:
    ...
    ax.plot(t_list, distances_list[:], label='Particle {}'.format(
        1), linewidth=0.5, color='black')
    ...
    plt.savefig('plots/single_particle_distances.{}.pdf'.format(run_i), dpi=300)
...
values_counter = 0
with open('r_histogram_single.dat', 'w') as f:
